<template>
  <div class="order-item">
    <div class="head">
      ...
      <!-- 未付款，倒计时时间还有{{ order.payLatestTime }} -->
      <span class="down-time" v-if="order.orderState === 1">
        <i class="iconfont icon-down-time"></i>
        <b>付款截止：{{ order.payLatestTime }}</b>
      </span>
      <!-- 已完成 已取消 -->
      <a href="javascript:;" @click="$emit('on-delete', order)" class="del" v-if="[5, 6].includes(order.orderState)">删除</a>
    </div>
    <!-- 商品信息 -->
    <div class="body">
      ...
      <div class="column state">
        ...
        <!-- 待评价：评价商品 -->
        <!-- 已完成：查看评价 -->
        <p @click="$emit('on-logist', order)" v-if="order.orderState === 3"><a href="javascript:;" class="green">查看物流</a></p>
...
export default {
  ...
  emits: ['on-cancel', 'on-delete', 'on-confirm', 'on-logist'],
  setup(props) {
    // eslint-disable-next-line vue/no-mutating-props
    props.order.countdown = 10000000000000
