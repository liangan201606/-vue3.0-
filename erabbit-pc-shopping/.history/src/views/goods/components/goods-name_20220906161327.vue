<template>
  ...
  <div class="g-service">
    ...
    <dl>
      <dt>配送</dt>
      <dd>至 <XtxCity :fullLocation="fullLocation" @change="changeCity"></XtxCity></dd>
    </dl>
    <dl>
      <dt>服务</dt>
      <dd>
...
export default {
  ...
  setup(props) {
    // 提供后台四项数据
    const provinceCode = ref('11000')
    const cityCode = ref('119900')
    const countryCode = ref('110101')
    const fullLocation = ref('北京市 市辖区 东城区')
    // 取出用户收货地址中默认的地址给四个数据(登录后)
    ...
    const changeCity = (result) => {
      ...
      cityCode.value = defaultAddre.cityCode
      countryCode.value = defaultAddress.countryCode
      fullLocation.value = defaultAddress.fullLocation
    }
    return { fullLocation }
