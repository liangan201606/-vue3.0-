  ...
  <Form class="xtx-form" ref="formCom" :validation-schema="schema" v-slot="{ errors }" autocomplete="off">
    ...
    <div class="xtx-form-item">
      <div class="field">
        ...
        <Field :class="{ error: errors.code }" v-model="form.code" name="code" class="input" type="text" placeholder="请输入验证码" />
...
export default {
  ...
  setup(props) {
        ...
        userQQBindLogin({ unionId: props.unionId, ...form })
          .then((data) => {
            ...
            store.commit('user/setUser', { id, account, avatar, mobile, nickname, token })
             // 合并购物车
             store.dispatch('car/mergeCart').then(() => {
              // 2. 跳转到来源页或者首页
    ...
    return { nickname, avatar, form, schema: mySchema, send, login, time, formCom }
  }
}
</script>
