...
<template>
  <Form class="xtx-form" ref="formCom" :validation-schema="mySchema" v-slot="{ errors }" autocomplete="off">
    ...
    <div class="xtx-form-item">
      <div class="field">
        <i class="icon iconfont icon-lock"></i>
        <Field :class="{ err: errors.rePassword }" name="rePassword" v-model="form.account" class="input" type="password" placeholder="请确认密码" />
      </div>
      <div v-if="errors.rePassword" class="error">{{ errors.rePassword }}</div>
    </div>
    <a href="javascript:;" class="submit" @click="login()">立即提交</a>
  </Form>
</template>
...
export default {
  ...
  setup(props) {
    ...
    return { form, mySchema, send, time, formCom, login }
  }
}
</script>
