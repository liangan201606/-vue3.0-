...
export default {
  ...
  setup() {
    ...
    const login = async () => {
      ...
      if (valid) {
        ...
        if (!isMsgLogin.value) {
          ...
          // 1.发送验证码
          // 1.1 绑定发送验证码按钮点击事件
          // 1.2校验手机号码 成功则发送短信(定义APi) 请求成功开启60s倒计时
          // 1.3如果失败 失败的样式显示出来
          // 2.手机号登录
          // 2.1准备一个手机号码登录
          // 2.2调用API函数
          // 2.3成功：存储用户信息 + 跳转至来源页或首页 + 消息提示
          // 2.4失败：消息提示
        }
      }
    }
    ...
    const { pause, resume } = useIntervalFn(() => {},1000,false)
    const send = async () => {
      // 校验手机号码是否符合规则
    ...
    return { isMsgLogin, form, schema: mySchema, login, formCom, send }
  }
}
</script>
