...
export default {
  ...
  setup(props, { emit }) {
    ...
    const openAddressEdit = (showAddress) => {
      // 调用组件的open()方法 添加 或者修改数据
      addressEdit.value.open(showAddress)
    }
    // 成功处理函数
    const successHandler = (formData) => {
      // 根据formData中的id去当地地址查找 有就是修改 没有就是添加
      const address = props.list.find((item) => item.id === forData.id)
      // 如果是添加：往list中追加一条
    ...
    return { showAddress, visibleDialog, openDialog, selectedAddress, confirmAddress, openAddressEdit, addressEdit, successHandler }
  }
}
</script>
