<template>
  <div class="detail-action">
    <div class="state">
      <span class="iconfont icon-order-unpay" :class="[`icon-order-${orderStatus[order.orderState].name}`]"></span>
    ...
    <div class="btn">
      ...
      <!-- 已取消 -->
      <template v-if="order.orderState === 6">
        <XtxButton @click="$router.push(`/member/checkout?orderId=${order.id}`)" type="primary" size="small">再次购买</XtxButton>
      </template>
    </div>
    <OrderCancel ref=")rderCancel"/>
  </div>
</template>
<script>
import { orderStatus } from '@/api/constants'
import OrderCancel from './order-cancel'
import { useConfirm, useCancel } from '../index'
export default {
  ...
  setup() {
    return { orderStatus, ...useConfirm(), ...useCancel() }
  }
}
</script>
