  ...
  <XtxDialog v-model:visible="visibleDialog" title="查看物流">
    <div class="order-logistics-goods" v-if="logistics">
      ...
      <div class="info" v-if="logistics.company">
        <p>
          <span>配送企业：{{ logistics.company.name }}</span>
        </p>
        <p>
          <span>快递单号：{{ logistics.company.number }}</span>
        </p>
        <p>
          <span>联系电话：{{ logistics.company.tel }}</span>
        </p>
      </div>
    </div>
    <div class="order-logistics-list" v-if="logistics.list">
      <div class="item" v-for="(item, i) in logistics.list" :key="item.id">
        <div class="line"></div>
        <div class="logistics">
          <p>{{ logistics.list.text }}</p>
          <p>{{ logistics.list.time }}</p>
        </div>
      </div>
    </div>
  </XtxDialog>
...
export default {
  ...
  setup() {
    ...
    const open = (order) => {
      ...
      // 查询物理信息
      receiptOrder(order.id).then((data) => {
